...
use App\Models\BarbarInfo;
use DB;
use Image;
use Validator;
use Illuminate\Support\Facades\Hash;

class BarbarController extends Controller
{
    public function index(Request $request)
    {
        $primaryInfo = PrimaryInfo::profession(2)
            ->with('addresses','barbar')
            ->active()
            ->orderBy('id', 'desc');
        $expertise = DB::table("specializations")->where('type', 2)->pluck('name', 'id');
        ...
        if(isset($name) || isset($mobile_no) || isset($designation)) {
            ...
            if(isset($designation)){
                ...
                $primaryInfo = $primaryInfo->whereHas('barbar',
        ...
        return view('barbar.index', compact('primaryInfo', 'expertise', 'name', 'mobile_no', 'designation'));
    }
    ...
    public function registrationForm()
    {
        ...
        return view('barbar.registration', compact('genders', 'religions', 'marital_status', 'divisions', 'blood_groups', 'exams', 'boards', 'expertise'));
    }
    ...
    public function registrationFormSubmit(Request $request)
    {
        ...
        $destination = public_path('/uploads/images/barbar');
    ...
    public function details(int $id)
    {
        ...
        $expertise = DB::table("specializations")->where('type', 2)
            ->pluck('name','id');
        return view('barbar.details', compact('primaryInfo', 'expertise'));
    }
}
