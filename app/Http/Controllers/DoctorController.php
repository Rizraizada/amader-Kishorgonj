...
class DoctorController extends Controller
{
    public function index(Request $request) {
        $primaryInfo = PrimaryInfo::profession(1)
            ->with('addresses','doctor')
            ->active()
            ->orderBy('id', 'desc');
        $expertise = DB::table("specializations")->where('type', 1)->pluck('name', 'id');
        ...
        if(isset($name) || isset($mobile_no) || isset($designation)) {
            ...
            if(isset($designation)) {
                $primaryInfo = $primaryInfo->whereHas('doctor',
                    function ($query) use ($designation) {
                        $query->whereIn('expertise', $designation);
                    });
            }
        }
        else
        ...
        return view('doctor.index', compact('primaryInfo', 'expertise', 'name', 'mobile_no', 'designation'));
    }
    ...
    public function registrationForm() {
        ...
        $specializations = DB::table("specializations")->where('type', 1)->pluck('name', 'id');
        $designations = DB::table("designations")->where('type', 1)->pluck('name', 'id');

        return view('doctor.registration', compact('genders', 'religions', 'marital_status', 'divisions', 'blood_groups', 'exams', 'specializations', 'designations', 'boards'));
    }
    ...
    public function registrationFormSubmit(Request $request) {
        ...
        $inputs['degree_name'] = array_values(array_filter($inputs['degree_name']));
        $inputs['diploma_institute'] = array_values(array_filter($inputs['diploma_institute']));
        $inputs['dip_start_date'] = array_values(array_filter($inputs['dip_start_date']));
        $inputs['dip_end_date'] = array_values(array_filter($inputs['dip_end_date']));
        ...
        $validator = Validator::make($inputs, [
            ...
            // 'institute_name.*' => 'max:255',
            // 'result' => 'required_with:institute_name | required',
        ...
        $destination = public_path('/uploads/images/doctor');
        ...
        $primaryInfo->doctor()->create($inputs);
        ...
        $diplomas = [];
        for($i = 0; $i < count($inputs['degree_name']); $i++) {
            if(isset($inputs['degree_name'][$i]) && isset($inputs['diploma_institute'][$i]) && isset($inputs['dip_start_date'][$i]) && isset($inputs['dip_end_date'][$i])) {
                $diplomas[] = [
                    'degree_name' => $inputs['degree_name'][$i],
                    'diploma_institute' => $inputs['diploma_institute'][$i],
                    'dip_start_date' => $inputs['dip_start_date'][$i],
                    'dip_end_date' => $inputs['dip_end_date'][$i]
                ];
            }
        }
        //\Log::info($diplomas);
        if($diplomas)
            $primaryInfo->diplomas()->createMany($diplomas);
    ...
    public function details(int $id) {
        ...
        $expertise = DB::table('specializations')->where('type', 1)->pluck('name', 'id');
        return view('doctor.details',compact('primaryInfo', 'expertise'));
    }
}
