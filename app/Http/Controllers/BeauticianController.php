...
class BeauticianController extends Controller
{
    public function index(Request $request) {
        $primaryInfo = PrimaryInfo::profession(5)
            ->with('addresses','beautician')
            ->active()
            ->orderBy('id', 'desc');
        $expertise = DB::table("specializations")->where('type', 5)->pluck('name', 'id');
        ...
        if(isset($name) || isset($mobile_no) || isset($designation)) {
            ...
            if(isset($designation)){
                $expertise_in = implode(',', $designation);
                $primaryInfo = $primaryInfo->whereHas('beautician',
                    function($query) use ($expertise_in){
                        $query->whereRaw("find_in_set($expertise_in, expertise)");
                    }
                );
            }
        }
        ...
        return view('beautician.index', compact('primaryInfo', 'expertise', 'name', 'mobile_no', 'designation'));
    }

    public function index2(Request $request)
    {
        $primaryInfo = [];
        if($request->has('name') || $request->has('mobile_no') || $request->has('designation')) {

            $name = $request->name ?? null;
            $mobile_no = $request->mobile_no ?? null;
            $designation = $request->designation ?? null;

            $expertise = DB::table("specializations")->where('type', 5)
            ->where('name', 'like', '%'.$designation.'%')
            ->pluck('id');
            $beauticianInfo = BeauticianInfo::whereIn('expertise', $expertise)->pluck('primary_info_id');

            $primaryInfo = PrimaryInfo::where('profession_type', 5)
            ->where('status', 0)
            ->with('addresses','beautician')
            ->where('name_en', 'like', '%'.$name.'%')
            ->orWhere('name_bn', 'like', '%'.$name.'%')
            ->orWhere('mobile_no', 'like', '%'.$mobile_no.'%')
            ->whereIn('id', $beauticianInfo)
            ->orderBy('id', 'desc')
            ->simplePaginate(9);
        } else {
            $expertise = DB::table("specializations")->where('type', 5)->pluck('name', 'id');
            $primaryInfo = PrimaryInfo::where('profession_type', 5)
            ->where('status', 0)
            ->with('addresses','beautician')->orderBy('id', 'desc')
            ->simplePaginate(9);
        }
        return view('beautician.index', compact('primaryInfo', 'expertise'));
    }
    ...
    public function registrationForm()
    {
        ...
        return view('beautician.registration', compact('genders', 'religions', 'marital_status', 'divisions', 'blood_groups', 'exams', 'boards', 'expertise'));
    }
    ...
    public function registrationFormSubmit(Request $request)
    {
        ...
        $destination = public_path('/uploads/images/beautician');
        ...
        $primaryInfo->beautician()->create([
            'expertise' => implode(',', $inputs['expertise']),
            'working_start_year' => $inputs['working_start_year']
        ]);
    ...
    public function details(int $id)
    {
        ...
        $expertise =  DB::table("specializations")->where('type', 5)
            ->pluck('name', 'id');
        return view('beautician.details', compact('primaryInfo', 'expertise'));
    }
}
