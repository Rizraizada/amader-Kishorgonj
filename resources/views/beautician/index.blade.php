@extends('layouts.layout')
    @section('content')
                ...
                <div class="container" style="text-align: center;">
                    <div class="inner-box">
                        <div class="text-box" style="position: relative;">
                            <h4 style="display: inline-block; font-size: 24px;">বিউটিশিয়ান অনুসন্ধান করুন!</h4>
                            <a href="/beautician/registration"   class="btn-style-1"></i>বিউটিশিয়ান রেজিস্ট্রেশন করতে ক্লিক করুন</a>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <section class="candidates-search-bar">
            <div class="container">
                <form action="/beautician" id="beauticianSearch" style="background-color: #c0c2c4;">
                   <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3">
                            <select class="form-control" name="designation[]" id="designation" required data-parsley-errors-messages-disabled>
                                <option value="">অভিজ্ঞতার ক্ষেত্র নির্বাচন করুন</option>
                        ...
                        <div class="col-md-4"style="margin-bottom: 15px;">
                            <div class="job-box" style="width: 350px; height: 200px;">
                                <div class="box" style="padding: 20px;/* margin: 20px; */display: flex;/* width: 100%; */height: 80%;">
                                    ...
                                    <a href="#"><img src="{{ asset('/uploads/images/beautician/'.$row->photo) }}" alt="img" style="max-width:100%; height: 100%; border-radius: 25px;"></a>
                                    </div>
                                    <div class="text-col" style="flex: 2;text-align: left;/* padding-left: 60px; */border-radius: 25px;padding-left: inherit;">
                                    <h4><a href="#/beautician/details/{{ $row->id}}" class="job-name" style="color: #ff6600;">{{ $row->name_bn }}</a></h4> <!-- Set color to orange (#ff6600) -->
                                    ...
                                    <h4><p><i class="" style="color: #009900;"></i> <span class="job-consultant" style="color: #009900;">
                                        @php
                                            if(isset($row->beautician->expertise))
                                                $beauticianExpertise = explode(',',$row->beautician->expertise);
                                        @endphp
                                            @if ($beauticianExpertise)
                                                @foreach ($beauticianExpertise as $key => $value)
                                                    {{ $expertise[$value] }},
                                                @endforeach
                                            @endif
                                    ...
                                    <p><i class="fa fa-clock"></i> <span class="job-experience">{{ (date('Y') - $row->beautician->working_start_year). ' Years' }}</span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('custom-js')
    <script>
        $(function () {
            $('#beauticianSearch').parsley().subscribe('parsley:form:validate', function (formInstance) {
