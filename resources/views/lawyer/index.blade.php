@extends('layouts.layout')
    @section('content')
                ...
                <div class="container" style="text-align: center;">
                    <div class="inner-box">
                        <div class="text-box" style="position: relative;">
                            <h4 style="display: inline-block; font-size: 24px;">আইনজীবী অনুসন্ধান করুন!</h4>
                            <a href="/lawyer/registration" class="btn-style-1"></i>আইনজীবী রেজিস্ট্রেশন করতে ক্লিক করুন</a>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <section class="candidates-search-bar">
            <div class="container">
                <form action="/lawyer" id="lawyerSearch" style="background-color: #c0c2c4;">
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3">
                            <select class="form-control" name="designation[]" id="designation" required data-parsley-errors-messages-disabled>
                                <option value="">মামলা পরিচালনার ক্ষেত্র নির্বাচন করুন</option>
                        ...
                        <div class="col-md-4"style="margin-bottom: 15px;">
                            <div class="job-box" style="width: 350px; height: 200px;">
                                <div class="box" style="padding: 20px;/* margin: 20px; */display: flex;/* width: 100%; */height: 80%;">
                                    ...
                                    <a href="#"><img src="{{ asset('/uploads/images/lawyer/'.$row->photo) }}" alt="img" style="max-width:100%; height: 100%; border-radius: 25px;"></a>
                                    </div>
                                    <div class="text-col" style="flex: 2;text-align: left;/* padding-left: 60px; */border-radius: 25px;padding-left: inherit;">
                                    <h4><a href="#/lawyer/details/{{ $row->id}}" class="job-name" style="color: #ff6600;">{{ $row->name_bn }}</a></h4> <!-- Set color to orange (#ff6600) -->
                                    <p class="pt-4"><i class="fa fa-phone"></i> <a href="tel:{{ $row->mobile_no }}">{{ $row->mobile_no }}</a> <span class="job-phone"></span></p>
                                    <h4><p><i class="" style="color: #009900;"></i> <span class="job-consultant" style="color: #009900;">{{ isset($row->lawyer->expertise) ? $expertise[$row->lawyer->expertise] : '' }}</span></p></h4> <!-- Set color to green (#009900) -->
                                    <p><i class="fa fa-map-marker" style="color: #009900;"></i> <span class="job-location" style="color: #009900;font-size: 11px;">
                                            @if($row->addresses)
                                                {{ $row->addresses('2')->first()->address ?? 'Not added yet.' }}
                                            @endif</span></p> <!-- Set color to green (#009900) -->
                                    <p><i class="fa fa-clock"></i> <span class="job-experience">{{ $row->lawyer->experiences ?? '5 Years' }}</span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('custom-js')
    <script>
        $(function () {
            $('#lawyerSearch').parsley().subscribe('parsley:form:validate', function (formInstance) {
